...
export type ObjectShorthandPropsRecord = Record<
  string,
  DefaultObjectShorthandProps | undefined
>;
...
> = IsSingleton<DefaultAs> extends false
  ...
  :
      | ({ as?: DefaultAs } & ObjectShorthandProps<
          React.PropsWithRef<JSX.IntrinsicElements[DefaultAs]>
        >)
      | {
          [As in AlternateAs]: { as: As } & ObjectShorthandProps<
            React.PropsWithRef<JSX.IntrinsicElements[As]>
          >;
        }[AlternateAs];
...
export type ComponentState<Shorthands extends ObjectShorthandPropsRecord> = {
  components?: {
    [Key in keyof Shorthands]-?:
      | React.ComponentType<
          NonNullable<Shorthands[Key]> extends ObjectShorthandProps<infer P>
            ? P
            : NonNullable<Shorthands[Key]>
        >
      | (NonNullable<Shorthands[Key]> extends AsIntrinsicElement<infer As>
          ? As
...
export type ObjectShorthandPropsCompat<
  TProps extends ComponentPropsCompat = {}
> = TProps &
...
> = {
  ...
  [key in keyof Omit<TSlots, "root">]: key extends keyof TProps
    ? TProps[key]
    : any;
} & {
  root: TRootProps;
...
export type ResolvedShorthandPropsCompat<T, K extends keyof T> = Omit<T, K> &
  {
    [P in K]: T[P] extends ShorthandPropsCompat<infer U>
      ? ObjectShorthandPropsCompat<U>
      : T[P];
  };
...
export type ComponentStateCompat<
  ...
  DefaultedPropNames extends keyof ResolvedShorthandPropsCompat<
    Props,
    ShorthandPropNames
  > = never
> = RequiredPropsCompat<
  ResolvedShorthandPropsCompat<Props, ShorthandPropNames>,
  DefaultedPropNames
>;
